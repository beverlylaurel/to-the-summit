...
public class SeaManager : MonoBehaviour
{
    ...
    void UpdateState()
    {
        ...
        // draining when the next bore arrives, they collide, and the pair counts
        // as a single swash. [SOURCE: Coastal Wiki, Swash zone dynamics]
        //
        // The period follows from the ballistic swash model rather than a made-up
        ...
        //
        // and the drain back is longer -- field measurements put the backwash at
        // 20-40% more than the uprush. Measured here: R = 0.69 m on a 5.8% shore
        // gives T_up 6.5 s and a full swash of 14.9 s against a 10 s wave period,
        // a ratio of 1.5 -- inside the observed 1-3 band, and it moves with the
        // sea state instead of being pinned to it.
    ...
    /// THE SWASH IS NOT A COSINE. It used to be `0.5 - 0.5 cos(2pi phase)`:
    /// symmetric, so the water withdrew exactly as fast as it arrived. A real
    /// swash rushes up and drains back slowly, and the ballistic model says why --
    /// going up it is a body thrown against gravity, so the height follows
    /// `s (2 - s)`: quick at the start, easing into the turn.
    ///
    /// MIRRORED IN `SeaCommon.hlsl`. The shader draws the foam and the wet sand
    /// from the same curve; two copies that drift apart would put the foam and
    /// the waterline in different places.
