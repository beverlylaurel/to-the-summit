#ifndef URP_PHYSICALLY_BASED_SKY_EVALUATION_INCLUDED
#define URP_PHYSICALLY_BASED_SKY_EVALUATION_INCLUDED
...
{
    return saturate(float2(cosChi*0.5 + 0.5, height / _AtmosphericDepth));
}
...
// Should roughly represent the longest line of sight you can have through the atmosphere
// Higher values increase coverage of each slice which decreases quality
...
void EvaluateDistantAtmosphere(float3 V, inout float3 skyColor, inout float3 skyOpacity)
{
    ...
    skyColor += SAMPLE_TEXTURE2D_LOD(_SkyViewLUT, s_linear_clamp_sampler, uv, 0).xyz * _CelestialLightExposure;
...
#endif // URP_PHYSICALLY_BASED_SKY_EVALUATION_INCLUDED